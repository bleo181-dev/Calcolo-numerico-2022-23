...
coeff = [log10(pi^7/4*max([exp(1) - 5, tan(1/2)])), cos(nthroot(abs(-0.7*exp(1)^-0.2),3)), 0, -sin(0.2 + exp(1)^-1.6), 0 , acos(3.7*10^-1) + 1/4, log(sqrt(pi^3)+2/3)];
X = linspace(-1.5, 1.3);
Y = (log10(pi^7/4*max([exp(1) - 5, tan(1/2)])))*X.^6 + (cos(nthroot(abs(-0.7*exp(1)^-0.2),3)))*X.^5 + (-sin(0.2 + exp(1)^-1.6))*X.^3 + (acos(3.7*10^-1) + 1/4)*X.^1 + log(sqrt(pi^3)+2/3)
plot(X,Y)
